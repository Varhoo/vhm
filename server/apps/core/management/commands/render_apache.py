...
def init(*args, **kwargs):
    ...
    if params[0] in args: #param apache
        ...
        for account in Account.objects.all():
            ...
            path = os.path.normpath("%s%s" % (APACHE_DIR_LOCATION,account.path))
            directory.append({"path":path,"user":slugify(account.name),'group':APACHE_GROUP})
            
            #get all users for creating
            users.append({"user":slugify(account.name),'group':APACHE_GROUP})
            ...
            for it in ApacheAlias.objects.filter(account=account.id, is_valid=True):
                if it.site:
                    ...
                    if it.django_wsgi:
                        processname = "%s_%d_%s" % ("wsgi",it.id, account.name)
