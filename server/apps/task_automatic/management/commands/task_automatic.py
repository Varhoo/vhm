...
def init(*args, **kwargs):
    ...
    for it in tasks:
        ...
        debug("%s (cron %s ) - last action before %s s." %
              (it.title, it.cron, it.date_last))
    ...
    for it in tasks:
        ...
        if it.status > 2:  # 2 = done, 3 = error
            ...
            subject = "ERROR: mixmotor import: %s - %s" % (
                it.title, it.common)
            msg = it.exit_result
    ...
    [it.delete()
     for it in Task.objects.filter(status=2).order_by("-date_run")[100:]]
